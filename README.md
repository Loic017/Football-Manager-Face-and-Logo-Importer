...
-   ~~Improved resizing: Aspect ratio and other methods to keep the image from stretching too much~~ DONE
-   Competition image importing
-   ~~Save graphics folder: No need to choose the graphics folder on each boot up~~ DONE
-   **Fix bug where you cannot import an image right after importing one (User has to restart program after every use)** CANT FIGURE IT OUT **YET**
    -   (The import type radio button is deleted? after its first used, Not sure why)
